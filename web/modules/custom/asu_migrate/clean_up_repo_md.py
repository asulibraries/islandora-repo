    ...
    for n in nstring:
        ...
        if len(lc.index) > 0:
            ...
            if m is None:
                ...
                {"label": val_to_query, "authority": authority, "uri": None}, ignore_index=True)
                return_strings.append(val_to_query)
                continue
            uri = m.group(0).replace("< x-uri: ", "").replace("\r\n<", "")
            print(uri)
            loc_df = loc_df.append(
                {"label": val_to_query, "authority": authority, "uri": uri}, ignore_index=True)
...
def main(argv):
    ...
    # print(att_df)
    merge_df['Model'] = merge_df.apply(lambda row: get_model(
        row['Attachment Count'], row['Item ID'], att_df, None), axis=1)
...
#    print(merge_df.iloc[0])
        ...
        if (not math.isnan(c['Item ID'])):
            ...
            if cid > 0:
                ...
                for index, a in atts.iterrows():
                    ...
                    new_row = {'Item ID': str(a['attachment id']), 'Item Title': a['attachment label'], 'Notes': a['attachment notes'], 'Model': get_model(1, None, att_df, str(a['attachment id'])), 'Parent Item': a['item id'], 'Visibility': a_status, 'System Created': a['file created'], 'System Updated': a['file created'], 'Attachment Count': 1, 'Complex Object Child': 1, 'Description': a['attachment description']}
                    # print("add att")
                    # print(new_row)
    ...
    for ccc in contribs:
        ...
        if 'Personal Contributor Role' in merge_df:
            ...
            if role in merge_df:
                merge_df[ccc] = merge_df[[ccc, role]].apply(lambda x: (
                    '|'.join(x.map(str) if not x.empty else '')), axis=1)
    ...
    corp_contribs = merge_df[merge_df.columns[pandas.Series(
        merge_df.columns).str.startswith('Institutional contributor')]]
    if corp_contribs.empty:
        corp_contribs = merge_df[merge_df.columns[pandas.Series(
            merge_df.columns).str.match('Institutional Contributor([^\s]\.?[0-9]*|$)$')]]
    ...
    att_df.to_csv('data/migration_data/att_file_' +
                  str(int(merge_df.iloc[0]['Collection ID'])) + '_v2.csv')
