 ...
 *
 * @FieldFormatter(
 *   id = "workbench_entity_reference_formatter",
 *   label = @Translation("Workbench Entity Relation CSV Formatter"),
 *   field_types = {
 *     "entity_reference"
 *   }
 * )
 */
class WorkbenchEntityReferenceFormatter extends EntityReferenceLabelFormatter {
  ...
  public function settingsSummary() {
    $summary = [];
    $summary[] = $this->t('Term Reference: @reference; Sub-Field delimiter: @subfield_delimiter', [
      '@reference' => $this->getSetting('term_identifier'),
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    foreach ($items as $delta => $item) {
      ...
      $string_value = (array_key_exists("#plain_text", $elements[$delta]) ? $elements[$delta]['#plain_text'] : $elements[$delta]["#title"]);
      ...
      $elements[$delta]['#markup'] = implode($subfield_delimiter, [$term->bundle(), $string_value]);
