...
class PerfArchiveChild {
  ...
  public static function alter(array &$form, FormStateInterface $form_state) {
    ...
    $form['actions']['submit']['#submit'][] = [static::class, 'create_files'];
    ...
    $form['actions']['submit_add_more']['#submit'][] = [static::class, 'submit_add_more'];
  }
   ...
   * Form submission handler.
   */
  public static function submit_add_more(array &$form, FormStateInterface $form_state) {
    $node_id = $form_state->getValue(static::NODE_COORDS);
    if ($node_id) {
   ...
   * Form submission handler.
   */
  public static function create_files(array &$form, FormStateInterface $form_state) {
    ...
    if (isset($form_file[0]) && !empty($form_file[0])) {
      ...
      $model = $form_state->getValue('field_model');
