...
/**
 * Class PerfArchiveDepositController.
 */
class PerfArchiveDepositController extends ControllerBase {
  ...
  public function content(NodeTypeInterface $node_type) {
    ...
    if ($this->currentUser->isAuthenticated()) {
      $form = \Drupal::entityTypeManager()->getStorage('webform')->load('performance_archive_add_children');
      $output[] = \Drupal::entityTypeManager()->getViewBuilder('webform')->view($form);
      $output[] = \Drupal::entityTypeManager()->getViewBuilder('webform')->view($form);
      $output[] = \Drupal::entityTypeManager()->getViewBuilder('webform')->view($form);
      $output[] = \Drupal::entityTypeManager()->getViewBuilder('webform')->view($form);
      $output[] = \Drupal::entityTypeManager()->getViewBuilder('webform')->view($form);
