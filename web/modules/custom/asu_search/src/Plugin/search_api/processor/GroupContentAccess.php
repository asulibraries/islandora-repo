 ...
 *   label = @Translation("Group-Based Content access"),
 *   description = @Translation("Adds group-based content access checks for nodes and comments."),
 *   stages = {
 *     "add_properties" = 0,
 *     "pre_index_save" = -10,
 *     "preprocess_query" = -30,
 ...
 * )
 */
class GroupContentAccess extends ProcessorPluginBase {
  ...
  protected function addNodeAccess(QueryInterface $query, AccountInterface $account) {
    ...
    foreach ($grants as $realm => $gids) {
      foreach ($gids as $gid) {
        ...
        if ($realm == "gnode_unpublished:asu_repository_item") {
          $unpublished_access_groups[] = $gid;
        }
        // $enabled_conditions->addCondition($node_grants_field_id, "node_access_$realm:$gid");
      }
    }
    ...
    foreach ($affected_datasources as $entity_type => $datasources) {
      foreach ($datasources as $datasource_id) {
        ...
        if ($entity_type == 'node' && $unpublished_own) {
          $author_field = $this->findField($datasource_id, 'uid',
            'integer'
          );
          if ($author_field) {
        ...
        if ($entity_type == 'node' && count($unpublished_access_groups) > 0) {
          foreach ($unpublished_access_groups as $tgid) {
            $enabled_conditions->addCondition($node_grants_field_id, "node_access_gnode_unpublished:asu_repository_item:$gid");
          }
        }
      }
    }
